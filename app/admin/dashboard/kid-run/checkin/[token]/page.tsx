...
export default function KidRunCheckinPage() {
  ...
  return <div className="mx-auto max-w-3xl p-4 md:p-8">
    <div className="rounded-lg border bg-white p-6 shadow-sm">
      ...
      <div className="mt-5 space-y-3">{application.participants.map((p: any, index: number) => <label key={p.id} className={`flex items-start gap-3 rounded-md border p-4 ${p.bibCollectedAt ? "bg-emerald-50" : "cursor-pointer"}`}><input type="checkbox" disabled={!!p.bibCollectedAt} checked={p.bibCollectedAt || selected.includes(p.id)} onChange={(e) => setSelected(e.target.checked ? [...selected, p.id] : selected.filter((id) => id !== p.id))} className="mt-1 h-5 w-5" /><div><div className="font-bold">BIB {index + 1}: {p.bibNumber} - {p.fullName}</div><div className="text-sm text-slate-600">{p.category.name} · {p.category.distanceLabel} · sinh năm {p.birthYear}</div>{p.shirts.length > 0 && <div className="mt-1 text-sm">Áo: {p.shirts.map((s: any) => `${s.styleName} size ${s.size}`).join(", ")} · {application.shirtPaymentStatus === "PAID" ? "Đã thanh toán" : "Chưa thanh toán"}</div>}{p.bibCollectedAt && <div className="mt-1 text-sm font-semibold text-emerald-700">Đã nhận lúc {new Date(p.bibCollectedAt).toLocaleString("vi-VN")}</div>}</div></label>)}</div>
