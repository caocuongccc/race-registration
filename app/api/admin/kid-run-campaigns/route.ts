...
export async function GET() {
  try {
    ...
    const campaigns = await prisma.kidRunCampaign.findMany({
      ...
      include: {
        _count: {
          select: { applications: true, categories: true, shirtStyles: true },
        },
      },
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (
      ...
      return NextResponse.json(
        { error: "Tên, slug, ngày tổ chức hoặc địa điểm không hợp lệ" },
        { status: 400 },
      );
    }
    const bankData =
    ...
    const campaign = await prisma.kidRunCampaign.create({
      data: {
        ...
        shirtBuyerNote:
          "Đăng ký áo là tự nguyện. Áo chỉ được ghi nhận sau khi có email xác nhận thanh toán thành công.",
        createdById: user.id,
