...
export async function POST(
  ...
  try {
    ...
    const unassigned = await prisma.kidRunParticipant.count({
      where: {
    ...
    if (unassigned)
      return NextResponse.json(
        { error: `Còn ${unassigned} runner chưa được xếp nhóm` },
        { status: 409 },
      );
    const issued = await prisma.$transaction(
      async (tx) => {
        const categories = await tx.kidRunRaceCategory.findMany({
          where: {
            campaignId: id,
            isAvailable: true,
        ...
        for (const category of categories) {
          const count = await tx.kidRunParticipant.count({
            where: {
          ...
          if (!count) continue;
          const allocation = await tx.$queryRaw<Array<{ startNumber: number }>>(
            Prisma.sql`UPDATE "kid_run_race_categories" SET "nextBibNumber" = "nextBibNumber" + ${count}, "updatedAt" = NOW() WHERE "id" = ${category.id} RETURNING "nextBibNumber" - ${count} AS "startNumber"`,
          );
    ...
    const emailWhere: any = {
      campaignId: id,
      status: "CONFIRMED",
      participants: { some: {}, every: { bibNumber: { not: null } } },
      emailLogs: retryFailed
        ? {
            none: { type: "BIB_ANNOUNCEMENT", status: "SENT" },
            some: { type: "BIB_ANNOUNCEMENT", status: "FAILED" },
          }
        : { none: { type: "BIB_ANNOUNCEMENT" } },
    };
    ...
    for (const application of applications) {
      try {
        await sendKidRunBibEmail(application.id);
        sent++;
      } catch (error) {
        console.error("Send Kid Run BIB email failed:", application.id, error);
        failed++;
      }
    }
    const remainingUnattempted = await prisma.kidRunFamilyApplication.count({
      where: {
        ...
        status: "CONFIRMED",
        participants: { some: {}, every: { bibNumber: { not: null } } },
        emailLogs: { none: { type: "BIB_ANNOUNCEMENT" } },
      },
    });
    ...
    return NextResponse.json(
      { error: error.message || "Không thể cấp và gửi BIB" },
