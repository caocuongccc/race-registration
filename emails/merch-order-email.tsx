...
export function MerchOrderEmail({
  order,
  campaign,
  secretCode,
  qrPaymentUrl,
    ...
    <Html>
      ...
      <Body style={main}>
        <Container style={container}>
          <Text style={title}>
            {paid ? "Đã nhận thanh toán" : "Đặt áo thành công"}
          </Text>
          <Text style={subtitle}>{campaign.name}</Text>
          <Text>
            Xin chào <strong>{order.fullName}</strong>, mã đơn của bạn là{" "}
          ...
          <Section style={orderBox}>
              ...
              <Text key={item.id} style={{ margin: "8px 0 0" }}>
                ...
                {item.category === "MALE"
                  ...
                  : item.category === "FEMALE"
                    ? "Nữ"
                    : "Trẻ em"}{" "}
                - {item.type === "SHORT_SLEEVE" ? "T-shirt" : "Singlet"} - Size{" "}
                {item.size} × {item.quantity}: {money(item.totalPrice)}
              </Text>
            ))}
            <Hr />
            ...
            {order.deliveryMethod === "SHIPPING" && order.shippingAddress && (
              <Text style={{ margin: "8px 0 0" }}>
                Địa chỉ nhận hàng: <strong>{order.shippingAddress}</strong>
              </Text>
            )}
            <Text style={{ margin: "12px 0 0", fontWeight: 700 }}>
              Tổng cộng: {money(order.totalAmount)}
            </Text>
          </Section>
          {secretCode && (
            <Section style={secretBox}>
              <Text style={{ margin: 0, color: "#9a3412" }}>
                Mã bí mật để tra cứu đơn hàng
              </Text>
              <Text style={secret}>{secretCode}</Text>
              <Text
                style={{ marginBottom: 0, fontSize: "13px", color: "#7c2d12" }}
              >
          ...
          {!paid && bankInfo && (
            <Section style={{ marginTop: "20px" }}>
              ...
              <Text>
                ...
                <br />
                Số tài khoản: <strong>{bankInfo.accountNumber}</strong>
                <br />
                Chủ tài khoản: <strong>{bankInfo.accountName}</strong>
                <br />
                Nội dung: <strong>{order.transferContent}</strong>
              </Text>
              {campaign.requireOnlinePayment ? (
                <Text style={{ color: "#b91c1c", fontWeight: 700 }}>
                  Vui lòng không thay đổi nội dung chuyển khoản để hệ thống tự
                  động nhận diện.
                </Text>
              ) : (
                <Text style={{ color: "#92400e", fontWeight: 700 }}>
                  Vui lòng ghi đúng nội dung chuyển khoản để ban tổ chức đối
                  soát nhanh.
                </Text>
              )}
            </Section>
