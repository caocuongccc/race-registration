...
-- CreateEnum
CREATE TYPE "EventImageType" AS ENUM (
    'BANNER',
    'COVER',
    'GALLERY',
    'SHIRT_MALE',
...
-- Migration: Add WebhookLog table
-- File: migrations/XXX_add_webhook_log.sql
-- Create WebhookLog table
CREATE TABLE
    IF NOT EXISTS "webhook_logs" (
...
COMMENT ON TABLE "webhook_logs" IS 'Log all webhook requests from payment providers';

COMMENT ON COLUMN "webhook_logs"."provider" IS 'Payment provider name: sepay, momo, vnpay, etc.';

COMMENT ON COLUMN "webhook_logs"."event" IS 'Event type: payment, refund, etc.';

COMMENT ON COLUMN "webhook_logs"."payload" IS 'Full webhook payload in JSON format';

COMMENT ON COLUMN "webhook_logs"."status" IS 'Processing status: SUCCESS, FAILED';

COMMENT ON COLUMN "webhook_logs"."errorMessage" IS 'Error message if status is FAILED';